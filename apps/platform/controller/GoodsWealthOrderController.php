...
class goodsWealthOrderController extends BaseController
{
    ...
    protected function _initialize()
    {
        parent::_initialize();
        $this->permissions();
        $this->assign('navbar', 'goodswealthorder');
    ...
    {
        $this->seo();
        return view('goods_smart_order_index');
    }
    ...
    public function publish()
    {
        ...
        if (!empty($id)) {
             $result = Db::name('goodsWealthOrder')->alias('gwo')
                ->join('user u','gwo.user_id = u.id','LEFT')
                ->join('goods_wealth gw', 'gwo.goods_wealth_id = gw.id','LEFT')
                ->field('u.username, u.tel, gw.name as goods_name, gw.rengouedu, gw.zhouqi, gw.fanxihuobi ,gw.apr_3, gw.apr_6, gw.apr_9, gw.apr_12, gwo.*')
                ->where('gwo.id',$id)
                ->find();
            
            $zhouqi_apr = $result['zhouqi']/10;
            $result['apr'] = $result["apr_{$zhouqi_apr}"];
            $result['type_title'] = '财富';
            $this->assign('rows', $result);
        }

        $this->seo();
        return view('goods_smart_order_publish');
    }
}
